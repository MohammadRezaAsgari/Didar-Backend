...
from schedule.api.v1.views import (InstructorScheduleByIdAPIView,
...
class TestInstructorScheduleListAPIView(APITestCase):
    ...
    def test_get_schedule_list_success(self):
        ...
        self.assertIn("created_at", json_response.get("data").get("results")[0])
        self.assertIn("updated_at", json_response.get("data").get("results")[0])
    ...
    def test_post_schedule_invalid_data_fail(self):
        ...
        self.assertEqual(
            json_response.get("error").get("code"),
            ErrorObject.SCHEDULE_OVERLAPS.get("code"),
        )
        self.assertEqual(
            json_response.get("error").get("msg"),
            ErrorObject.SCHEDULE_OVERLAPS.get("msg"),
        )
    ...
    def test_resolve_url(self):
        ...
        self.assertEqual(resolver.view_name, "schedule:v1:instructor_schedule_list")
        self.assertEqual(resolver.func.view_class, InstructorScheduleListAPIView)
...
class TestInstructorScheduleByIdAPIView(APITestCase):
    def setUp(self):
        ...
        self.url = reverse(
            ...
            kwargs={"schedule_code": self.schedule.code},
        )
    ...
    def test_get_schedule_success(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(json_response.get("success"), True)
        self.assertEqual(json_response.get("data").get("title"), self.schedule.title)
    ...
    def test_get_schedule_not_found_fail(self):
        ...
        url = reverse(
            "schedule:v1:instructor_schedule",
            kwargs={"schedule_code": self.schedule2.code},
        )
    ...
    def test_delete_schedule_success(self):
        ...
        self.assertEqual(response.status_code, 204)
        self.assertEqual(
            Schedule.objects.filter(code=self.schedule.code).exists(), False
        )

    def test_resolve_url(self):
        resolver = resolve("/api/v1/instructor/schedules/5/")
        self.assertEqual(resolver.view_name, "schedule:v1:instructor_schedule")
        self.assertEqual(resolver.func.view_class, InstructorScheduleByIdAPIView)
        self.assertEqual(resolver.namespace, "schedule:v1")
...
class TestScheduleByInstructorAPIView(APITestCase):
    def setUp(self):
        ...
        self.url = reverse(
            ...
            kwargs={"instructor_id": self.instructor.id},
        )
    ...
    def test_resolve_url(self):
        ...
        self.assertEqual(resolver.view_name, "schedule:v1:schedule_list_by_instructor")
        self.assertEqual(resolver.func.view_class, ScheduleByInstructorAPIView)
        self.assertEqual(resolver.namespace, "schedule:v1")
